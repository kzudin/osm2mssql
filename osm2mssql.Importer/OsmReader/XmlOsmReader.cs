            ...
            using (var file = File.Open(fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                using (var reader = XmlReader.Create(file))
                {
                    ...
                    while (reader.IsStartElement())
                    {
                        ...
                        foundFirstElement = true;
                        var node = new Node(long.Parse(reader.GetAttribute("id")), 
                                            double.Parse(reader.GetAttribute("lat"), CultureInfo.InvariantCulture), 
                                            double.Parse(reader.GetAttribute("lon"), CultureInfo.InvariantCulture),
                                            new List<Tag>());
